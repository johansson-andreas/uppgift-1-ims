...
const resolvers = {
  Query: {
    ...
    totalStockValueByManufacturer: async () => {
      try {
        ...
        const mappedStockValue = await Promise.all(
          totalStockValue.map(async (manufacturer) => {
            const manufacturerName = await Manufacturer.findById(
              manufacturer._id
            );
    ...
    manufacturers: async () => {
      const manufacturer = await Manufacturer.find().populate("contact");
      return manufacturer;
    },
  },
  ...
  Mutation: {
    addProduct: async (
      _p: any,
      {
        ...
        input: {
          ...
          ManufacturerInput: ManufacturerType;
    ...
    updateProduct: async (
      _p: any,
      { input }: { input: ProductTypePartial }
    ) => {
